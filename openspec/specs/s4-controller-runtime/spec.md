# s4-controller-runtime Specification
...
Every persisted `plan/_s4` commitment, architecture, shard, link, lint, candidate, and review checkpoint SHALL validate against its closed contract and record the hashes of the authoritative parent inputs from which it was produced. The controller SHALL atomically maintain closed `plan/_s4/s4_state.json` control metadata containing the current phase, frozen input/config/prompt hashes, accepted artifact references, attempt and repair counters, per-work-package shard redo counters, pending Agent call, and seen issue signatures. Resume SHALL reconcile state, immutable checkpoints, and durable LLM trace evidence, reuse only the longest valid prefix whose parent hashes still match, continue from the first missing or invalid checkpoint, and never treat `plan/_s4` as a downstream semantic source. Once the S4 seal is valid and `stages.s4=done`, replay SHALL verify the seal even when the Run already has a terminal marker, then return as a read-only no-op. (Design 7.1.1: §4.8, §5.6.6, §6.4.2, §6.4.7; D1.4; M1-4c.)
...
Frozen-input drift, context overflow, invalid or truncated structured output after the existing one format repair, exhausted semantic budget, invalid shard or architecture after its allowed repair, critic non-convergence, and full-lint failure SHALL preserve diagnostic checkpoints, mark S4 failed through the existing controlled-exit path, and publish no consumable Plan or active pointer. Checkpoint or seal damage, immutable publication conflict, path/atomic-store failure, internal invariant violation, and deterministic tool defect SHALL remain `internal_error`, not controlled planning failures. (Design 7.1.1: §4.7-§4.8, §6.4.3-§6.4.7; D1.10; M1-4c.)
