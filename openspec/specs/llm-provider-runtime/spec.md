...
For a request with a JSON Schema, the runtime SHALL prefer a provider-native JSON/schema mode when that adapter has explicit support, otherwise SHALL embed the Schema in the prompt, extract the first JSON value from the returned text, and validate it with `jsonschema`. Native and fallback paths SHALL expose the same validated result contract. (Design: §8.4 item 2; `design.md` Decision 6; P8.)

#### Scenario: Native structured output validates
- **WHEN** the selected adapter explicitly supports native structured output and returns a value conforming to the requested Schema
- **THEN** the runtime returns that value in `parsed` and marks validation `pass`

#### Scenario: Fallback extracts the first JSON value
- **WHEN** native structured output is unavailable and the response contains surrounding prose followed by a Schema-valid JSON value
- **THEN** the runtime extracts the first complete JSON value, validates it, and returns it in `parsed` with validation `pass`

#### Scenario: One repair succeeds
- **WHEN** the first structured response cannot be parsed or fails Schema validation
...
- **WHEN** the single repair response also cannot be parsed or fails Schema validation
