...
__all__ = [
    ...
    "validate_activation_binding", "build_epoch_context", "plan_epoch_materialization", "project_version_binding", "attribute_pending_repair",
...
def __getattr__(name: str):
    ...
    if name in {"classify_migration", "project_plan_state", "project_file_ledger", "validate_revision_ledger", "validate_file_ledger", "validate_plan_successor", "successor_pointer", "validate_activation_binding", "build_revision_entry", "append_lease_finished", "append_lease_started", "append_revision_entry", "append_verification_committed", "latest_activation", "build_event_entry", "append_epoch_materialized"}:
        from .plan_revision import append_epoch_materialized, append_lease_finished, append_lease_started, append_revision_entry, append_verification_committed, build_event_entry, build_revision_entry, classify_migration, latest_activation, project_file_ledger, project_plan_state, successor_pointer, validate_activation_binding, validate_file_ledger, validate_plan_successor, validate_revision_ledger
        return {"classify_migration": classify_migration, "project_plan_state": project_plan_state, "project_file_ledger": project_file_ledger, "validate_revision_ledger": validate_revision_ledger, "validate_file_ledger": validate_file_ledger, "validate_plan_successor": validate_plan_successor, "successor_pointer": successor_pointer, "validate_activation_binding": validate_activation_binding, "build_revision_entry": build_revision_entry, "append_lease_finished": append_lease_finished, "append_lease_started": append_lease_started, "append_revision_entry": append_revision_entry, "append_verification_committed": append_verification_committed, "latest_activation": latest_activation, "build_event_entry": build_event_entry, "append_epoch_materialized": append_epoch_materialized}[name]
    if name in {"build_epoch_context", "plan_epoch_materialization", "project_version_binding", "attribute_pending_repair"}:
        from .materialization import attribute_pending_repair, build_epoch_context, plan_epoch_materialization, project_version_binding
        return {"build_epoch_context": build_epoch_context, "plan_epoch_materialization": plan_epoch_materialization, "project_version_binding": project_version_binding, "attribute_pending_repair": attribute_pending_repair}[name]
