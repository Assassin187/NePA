...
def _screened(initial_failures):
    gates = {f"arch_{index:02d}": {"initial_passed": True} for index in range(1, 11)}
...
def test_screening_rejects_two_repeated_initial_gate_failures_but_not_one():
    one = _screened([])
    ...
    assert _screen_model(two)["screening_pass"] is False
