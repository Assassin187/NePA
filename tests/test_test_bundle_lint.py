...
def test_bundle_lint_canonical_json_is_compact_sorted_utf8_without_newline():
    assert canonical_json_bytes({"z": "值", "a": 1}) == '{"a":1,"z":"值"}'.encode("utf-8")


def test_bundle_lint_rejects_non_finite_canonical_value():
    with pytest.raises(ValueError):
        canonical_json_bytes({"value": float("nan")})


def test_bundle_lint_gold_coverage_uses_task_and_s7_cases():
...
def test_bundle_lint_gold_coverage_rejects_uncovered_requirement():
...
def test_bundle_lint_gold_without_manifest_reports_skipped_warning():
