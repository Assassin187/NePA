...
| MODEL-004 两模型各四个公开工具会话 | `runs/qwen-p0-p2-candidate-12/public-tool-results.json` | 最新完整前置候选的八个真实会话通过；已有失败候选保持原记录 |
| MODEL-004 同最终候选三个空工程 MQTT 样本 | candidate-12／`20260913T101739Z-78273787` | 第一轮在 provider 调用期间中断，`run.json` 为 `interrupted`；其余两轮未执行，未完成 |
| MODEL-004 同候选一个空工程 HTTP 样本 | candidate-10 曾并行执行但失败；candidate-12 未执行 | 未完成 |
...
候选 7–12 的权威状态以各自 `experiment.json` 与 `runs/qwen-e2e/<run-id>/run.json` 为准。candidate-10 的两次 MQTT 与一次 HTTP 并行运行均在 final-integration 失败；candidate-12 是当前最新候选，能力、公开工具、私有修复及冻结通过，但 MQTT 首轮中断。因此 P0–P2 仍未通过真实验收，离线套件通过不得替代上述门槛。
