...
TR-4 的 full lint 输出预算采用唯一机械公式：`projected_output_tokens = 4000 × count(s6_owned 交付文件) + ceil(canonical_empty_output_envelope_bytes / 4)`，其中 `4000 = 400 行/文件 × 10 token/行`。空内容输出信封使用正常 Coder/Fixer 结构、实际规范化路径、空 `content`/`notes` 和主文档定义的 canonical JSON 编码；冻结输出预算取当前配置解析出的普通 Coder 与主 Fixer 路由有效 `max_tokens` 较小值。投影严格大于该值时直接命中预算溢出；运行期截断只从 provider 返回的类型化 `finish_reason` 及其不可变调用/attempt 引用重算，不从错误文本或 Agent 结论猜测。
...
| 2.0.3 | 2026-09-09 | 同步主文档 8.0.3，冻结 TR-4 完整输出预算公式、空输出信封口径、Coder/Fixer 路由上限比较及类型化截断证据来源 | 用户批准的 M1-10 修补计划 |
